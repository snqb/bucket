...
const LangToggle: FC = () => {
  const lang = useLang();
  return (
    <button className="text-xs text-text-dim hover:text-text font-mono"
...
const ThemeToggle: FC = () => {
  ...
  return <button className="text-xs text-text-dim hover:text-text" onClick={cycle} title={`Theme: ${theme}`}>{icon}</button>;
};
...
const Bucket: FC = () => {
  ...
  return (
    <div className="min-h-dvh flex flex-col">
      ...
      <header className="flex items-center justify-between p-3 border-b border-border safe-top safe-x">
        <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-3">
          ...
          <ThemeToggle />
          {canInstall && (
            <button className="text-xs text-text-dim hover:text-text" onClick={install} title={t("install")}>📲</button>
          )}
          {isIOS && !isStandalone && !canInstall && (
            <span className="text-[10px] text-text-dim">{t("iosHint")}</span>
          )}
          <button className="text-sm text-text-dim hover:text-text px-1" onClick={() => setShowSettings(true)} title={t("sync")}>⚙</button>
        </div>
      </header>
